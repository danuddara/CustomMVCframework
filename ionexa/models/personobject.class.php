 ...
 */
class PersonObject extends DBObject
{
     public function __construct($registry) {
         ...
         public function checkPerson($uid)
        {
          ...
          $sql = 'SELECT `Id`,`UserId`,`Type`,`Name`,`Description` FROM favouriteperson WHERE UserId='.$uid; // next time don't use from to dedine a coloum name
          $rows=$db->select($sql);
          return $rows;
        } 
        
        public function addPerson($userid=null,$name=null,$type=null,$description=null)
        {
             $suc=false;
             $db = $this->registry->getObject('db');
        ...
        public function updatePerson($name=null,$type=null,$description=null,$whereid=null)
        {
             $suc=false;
             $db = $this->registry->getObject('db');
             $coloums =  $this->columns; // keep a temprory copy so that we can use user object eaach time
